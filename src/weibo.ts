...
function replaceLinks(): void {
    ...
    if (urls.length > 0) {
        const requestNum = Math.floor(urls.length / 20) + (urls.length % 20 === 0 ? 0 : 1);
        for (let i = 0; i < requestNum; i++) {
            ...
            browser.runtime.sendMessage({ urls: requestUrls }).then(urls => {
                for (let i = 0; i < urls.length; i++) {
                    ...
                    document.querySelectorAll(`a[href="${url.url_short}"]`).forEach(l => {
...
browser.runtime.onMessage.addListener((urls: { url_short: string; url_long: string; type: number }[]) => {
